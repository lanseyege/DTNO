...
def build_predictor(model, model_name: str, info: dict) -> Predictor:
    """Dispatch on the model's KIND; refuse to silently treat a network as a
    baseline.

    The old version listed names: dt_fno / sg_dt_fno -> Direct, ar_fno -> AR,
    ANYTHING ELSE -> BaselinePredictor. That final fallback is right for
    persistence and the climatologies, which are not in MODEL_REGISTRY, and
    catastrophic for a neural model whose name the list has not been taught:
    `BaselinePredictor.predict` calls `self.baseline.predict(x, int(h))`, so a
    DirectTimeFNO received the integer horizon where it expected tau and raised

        AttributeError: 'int' object has no attribute 'reshape'

    from inside `time_embedding.features` --- three epochs into training, at the
    first horizon validation, with a traceback that points at the time
    embedding rather than at the dispatch that mis-routed the model.

    So: registered models dispatch by kind, and an unregistered object that is
    nevertheless an nn.Module is an error rather than a baseline.
    """
    from models import MODEL_REGISTRY

    kind = MODEL_REGISTRY.get(model_name)
    if kind == "direct":
    ...
    if isinstance(model, torch.nn.Module) and not hasattr(model, "n_model_evals"):
        raise ValueError(
            f"'{model_name}' is an nn.Module but is not in MODEL_REGISTRY, so "
            f"it would be evaluated as a retrieval BASELINE: predict(x, int(h)) "
            f"instead of predict(x, tau, grid).\n  Known models: "
            f"{sorted(MODEL_REGISTRY)}.\n  Register it in models/__init__.py "
            f"and training/tasks.py picks it up too.")
